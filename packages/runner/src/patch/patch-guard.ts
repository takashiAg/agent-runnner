...
export function inspectPatch(patch: string, config: RunnerConfig): PatchGuardResult {
  ...
  const denyMatchers = config.patch.denylist.map((pattern) => new Minimatch(pattern, { dot: true }));
